...
<div class="col-6 col-md-4 col-lg-4 mt-3">
    <div class="agenda-card shadow-sm rounded-4 overflow-hidden h-100 d-flex flex-column">
        <img src="{{ $agenda->gambar_agenda ?? 'https://placehold.co/600x300/198754/FFFFFF?text=Info+Acara' }}" alt="{{ $agenda->judul_agenda }}" class="card-img-top" style="height: 12rem; object-fit: cover;">
        <div class="card-body d-flex flex-column p-4">
            ...
            <div class="mt-auto">
                ...
                <button class="btn btn-success w-100 py-2 fw-bold text-white open-modal-btn" data-title="{{ $agenda->judul_agenda }}" data-date="{{ dateFormatToID($agenda->waktu_mulai) }} - {{ dateFormatToID($agenda->waktu_akhir) }}" data-location="{{ $agenda->lokasi }}" data-description="{{ $agenda->deskripsi ?? 'Tidak ada deskripsi' }}">
                    LIHAT DETAIL
                </button>
            </div>
        </div>
